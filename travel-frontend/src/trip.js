class Trip {
  ...
  static newTripDb(data){
    return fetch(`http://localhost:3000/api/trips`, {
      ...
      body: JSON.stringify({"name":data["name"], "description":data["description"], "user_id":data["user_id"], "city":data["city"], "state":data["state"], "country":data["country"], "start_date":data["start_date"], "end_date":data["end_date"], "leisure":data["leisure"], "work":data["work"], "beach":data["beach"], "family":data["family"], "friends":data["friends"]})
    })
      .then(resp => resp.json())
  ...
  static update(data){
    return fetch(`http://localhost:3000/api/trips/${data["id"]}`, {
      method: "PATCH",
      headers: {'Content-Type': 'application/json', 'Accept': 'application/json'},
      body: JSON.stringify({"name":data["name"], "description":data["description"], "user_id":data["user_id"], "city":data["city"], "state":data["state"], "country":data["country"], "start_date":data["start_date"], "end_date":data["end_date"], "leisure":data["leisure"], "work":data["work"], "beach":data["beach"], "family":data["family"], "friends":data["friends"]})
    }
  ...
  updateShow(){
    ...
    document.getElementById("trip-location").innerText = `${this.city}, ${this.state}, ${this.country}`
    document.getElementById("edit").dataset.id = this.id
    document.getElementById("trip-map").src = `https://www.google.com/maps/embed/v1/place?key=AIzaSyAv6zJzukQ3qrRUXJ1fGrHwd-6jq0hb-u0&q=${this.city}+${this.state}+${this.country}`
    ...
    document.getElementById("trip-tags").innerHTML =
    `Tags: <strong>${tagText}</strong>`
